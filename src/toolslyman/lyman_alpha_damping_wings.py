...
def compute_wavelength_bounds(z_source, dz):
    lam_min = 1215.67 * (1 + z_source - dz/2)
    lam_max = 1215.67 * (1 + z_source + dz/2)
...
def optical_depth_lyA_along_skewer(z_source,dz, xHI, dn, dr, temp=1e4*u.K, X_H=0.76, cosmo=None, f_alpha=0.4164, lambda_bins=1000, xHI_limit=1e-4, damped=True, verbose=False):
    """
    ...
    if isinstance(lambda_bins,(int,float)):
        lam_min, lam_max = compute_wavelength_bounds(z_source, dz)
        #limits converted back into rest frame
        lambda_obs = np.linspace(lam_min/(1 + z_source), lam_max/(1 + z_source), lambda_bins)*u.AA*(1+z_source)
